...
Обновить до нового тега — поменять `#v1.2.1` в `manifest.json` и вернуться в редактор. Если тег не указан, для повторного скачивания придётся удалить запись пакета из `packages-lock.json`: без этого Unity держится за ранее зафиксированный хеш.
...
Автотесты плагина гоняются без Unity: `Tests/run.sh`. Нужен .NET SDK 8. Прогоняются компиляция в пяти конфигурациях, проверка того, что заглушки не компилируются внутри Unity, проверка вырезаемости GizmoLazy из релиза по длине IL, компиляция всех примеров из README пакета и 230 кейсов по его логике.
