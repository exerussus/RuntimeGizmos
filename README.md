...
Автотесты плагина гоняются без Unity: `Tests/run.sh`. Нужен .NET SDK 8. Прогоняются компиляция в пяти конфигурациях, проверка того, что заглушки не компилируются внутри Unity, проверка вырезаемости GizmoLazy из релиза по длине IL, компиляция всех примеров из README пакета и 255 кейсов по его логике.
