...
Покрытие: ASCII 32–126, кириллица А–я, Ё и ё. Символ вне покрытия рисуется пустым квадратом, а не пропадает молча.
...
Слой нужен там, где рисование надо **пересчитывать каждый кадр**. Статичную геометрию на время уже закрывает `Gizmo.duration` в базе, без аллокаций.
