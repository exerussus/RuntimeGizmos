...
`Scope` снимает и возвращает **всё** состояние рисования: `color`, `matrix`, `lineWidth`, `depthTest`, `duration`, `dash` и накопленную фазу пунктира. Перегрузки отличаются только тем, что выставляют сразу после снимка, — возвращают они одинаково.

**Состояние глобальное.** Это статика на весь процесс, а не поле компонента: выставленный и не возвращённый `dash` или `depthTest` достанется всему последующему коду в кадре, включая чужой, который об этом не знает и ждёт сплошную линию. Поэтому `Scope` — не украшение, а способ не сломать соседа; если состояние меняется вне `Scope`, возвращать его нужно руками.

При **выключенном Domain Reload** (Enter Play Mode Options) статика переживает выход и повторный вход в Play Mode, поэтому пакет сам сбрасывает состояние на каждой границе Play Mode — новая сессия и эдит-мод после неё всегда начинаются с дефолтов, а не с того, что осталось от предыдущего запуска.
...
Колонки выравниваются по самой широкой ячейке, поэтому подгонять ширины пробелами не нужно. Блок прижимается к одному из девяти якорей.

```csharp
var hud = Gizmo.Table(GizmoAnchor.TopRight);
hud.Columns(GizmoTextAlign.Left, GizmoTextAlign.Right);
hud.Title("player");
hud.Row("hp", 100f, "F0");
hud.Row("ammo", 24);
hud.Separator();
hud.Row("grounded", true);
hud.Row("velocity", direction, "F2");
```

`Columns` можно не звать: колонок станет столько, сколько ячеек в самой длинной строке, все влево.

Числовые перегрузки `Row` форматируют значение **прямо в буфер отрисовки**, минуя `ToString()`. В отладочном выводе это главный источник мусора, и в профайлере он маскирует ровно то, ради чего HUD и включали.

Якорей девять — четыре угла, четыре середины сторон и центр:

```csharp
Gizmo.DrawScreenText("по центру", GizmoAnchor.Center);
Gizmo.DrawScreenText("снизу", GizmoAnchor.Bottom, 18f);
var stats = Gizmo.Table(GizmoCorner.BottomLeft);   // привычный угол тоже принимается
stats.Row("fps", 60);
```

Блоки на одном якоре укладываются стопкой в порядке вызова, вперемешку с `DrawScreenText`. У средних якорей (`Left`, `Center`, `Right`) стопка центрируется по вертикали, у остальных — отступает от края на `GizmoSettings.ScreenSafeArea`.

Хендл `GizmoTable` живёт **один кадр**. Сохранённый в поле и использованный на следующем кадре, он просто перестаёт что-либо делать — проверить можно через `IsValid`.

Раскладка считается на границе кадра, а не в момент вызова: ширину колонки нельзя знать, пока не поданы все строки. Край экрана при этом подставляет шейдер по размеру таргета текущей камеры, поэтому один и тот же блок корректен и на главном экране, и в `RenderTexture` другого размера.

`Gizmo.Table` — единственный метод пакета без `[Conditional]`, и по-другому быть не может: условный метод обязан возвращать `void`. Вырезается не вызов, а тело — в релизе возвращается пустой хендл, а все `Row` вырезаются как обычные `Draw*`, вместе с вычислением аргументов.
...
Эти секунды отмеряются часами эдит-мода, а не стенными: шаг часов зажат, поэтому сколь угодно долгая пауза редактора — переключение окна, потеря фокуса, импорт ассета, компиляция — считается за один обычный кадр. Иначе продюсер геометрии (`Update` компонента с `[ExecuteAlways]`, код редактора) молчал бы вместе с редактором, стенное время уходило бы за таймаут, и первый же тик после паузы гасил снимок — на экране это выглядит как мигание при переключении окон. Второе правило того же порядка: снимок не гаснет, пока его хоть раз не забрала камера, — геометрия не имеет права протухнуть, ни разу не появившись на экране. По-настоящему замолчавший продюсер гаснет как раньше, за `EditorStaleTimeout` секунд живого редактора.

**Кто вообще рисует в эдит-моде.** Продюсер геометрии — это `Update` компонента с `[ExecuteAlways]` или вызов из кода редактора, а player loop редактор крутит сам только когда сцена «шевелится»: курсор над Scene View, изменение объекта, анимация. Уводишь курсор в Game View, Inspector или вообще в другое приложение — тики прекращаются и рисовать становится некому. Scene View это скрывает: он не перерисовывается, на экране остаются прошлые пиксели. Game View перерисовывается и честно показывает пустоту — отсюда «пропадает именно в Game».

Поэтому пока продюсер жив, пакет сам просит редактор о тике (`EditorApplication.QueuePlayerLoopUpdate`): player loop крутится независимо от курсора, `Update` вызывается, геометрия в каждом кадре свежая. Ставка включается по первым же данным и снимается после пяти секунд живого редактора без них, но и без неё редактор получает редкий пробный тик — четыре раза в секунду. Без пробного тика получался бы замкнутый круг: продюсер не тикает, значит данных нет, значит ставку не включить. Это тот самый случай, когда курсор лежит в Game View с самого открытия сцены: в Scene View достаточно шевельнуть мышью, а в Game шевелить нечем.

В фоне (окно Unity неактивно) редактор не будится вообще, и часы эдит-мода на это время останавливаются: снимок не гаснет, пока человек в другом приложении. Иначе он вернётся, наведёт курсор на неактивное окно — и Game View перерисуется пустым.

Цена — редактор в эдит-моде не простаивает, пока идёт отрисовка. Если это мешает, всё вместе выключается через `GizmoSettings.EditorDriveUpdate = false` (тогда геометрия в Game View снова живёт только пока курсор над Scene View).
...
Страница в **Project Settings** (или **Tools → RuntimeGizmos → Настройки**) правит тот же самый ассет: своего хранилища у неё нет намеренно, иначе появилось бы два источника правды и настройки в билде расходились бы с показанным в окне. Если ассета нет, страница предлагает его создать и честно пишет, что пока работают платформенные дефолты.

Сверх инспектора ассета на странице два блока, которым в инспекторе делать нечего:

- **Профиль платформы** — `GizmoSettings.PlatformOverride`: посмотреть чужой профиль (Mobile, Web, Console, XR), не собирая билд и не переключая build target. Подмена живёт одну сессию редактора и сбрасывается на входе в Play Mode.
- **Диагностика Edit Mode** — живые значения, обновляются десять раз в секунду: активный конвейер (и предупреждение, если это Built-in), поднят ли рендерер, на месте ли подписка на камеры и узел `PlayerLoop`, просим ли мы редактор крутить player loop или идут только пробные тики, идут ли часы эдит-мода (в фоне они стоят), сколько прошло с последних команд `Draw*` при текущем `EditorStaleTimeout` и сколько живых регистраций у `GizmoLazy`. Это ровно те шесть чисел, по которым видно, почему в Scene View или Game View пусто.

Редакторный код живёт в отдельной сборке `RuntimeGizmos.Editor` (папка `Editor/`), в рантайм-сборку он не попадает: туда же переехали драйвер полей `Use/Value` и пункт меню создания ассета.
...
GizmoSettings.EditorDriveUpdate    = true;   // Edit Mode: крутить player loop, пока кто-то рисует
GizmoSettings.ScreenSafeArea       = 12f;    // отступ экранных блоков от краёв, в пикселях
GizmoSettings.HudColumnGap         = 1f;     // просвет между колонками таблицы, в ширинах символа
GizmoSettings.HudMaxCells          = 512;    // потолок ячеек экранной раскладки на кадр
GizmoSettings.HudMaxChars          = 8192;   // потолок арены символов раскладки, в char
```

`HudMaxCells` и `HudMaxChars` читаются один раз, при первом обращении к экранной раскладке в сессии: буферы выделяются под них сразу и дальше только переиспользуются. Переполнение роняет лишнее и один раз пишет в консоль.
